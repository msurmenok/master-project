...
def memetic_experimental9(num_creatures, NUM_GENERATIONS, services, hosts, MAX_PRIORITY, distance_to_cloud, mlmodel_filepath):
    num_services = len(services)
    num_hosts = len(hosts)
    ...
    cost_solution = report_best_population(pareto_head, hosts, services, num_hosts, num_services, mlmodel_filepath)
    return cost_solution[0][0]


def report_best_population(pareto_head, hosts, services, h_size, s_size, mlmodel_filepath):
    ...
    for i in range(pareto_size):
        if (fronts_best_P[i] == 1):
            ...
            cost_solution.append((solution, predicted_value, cost))
...
def test_memetic():
    ...
    placement = memetic_experimental9(num_creatures, NUM_GENERATIONS, services, hosts, MAX_PRIORITY, 18200, 'mlmodel.joblib')
