...
def single_placement(num_creatures, num_generations, folder_results, folder_data, mlmodel_filepath):
    ...
    nodeResources = list()
    servicesResources = list()
    gatewaysDevices = list()
    cloudId = -1
    distance_to_cloud = 18200
    appsDeadlines = list()

    # iterate over users to populate gateway devices
    #
    # index = 0
    # for app_num, app in enumerate(appsRequests):
    #     for instance, gw_id in enumerate(appsRequests[app_num]):
    #         for module in list(apps[app_num].nodes):
    ...
    #             x = nodeResources[gw_id]['x']
    #             y = nodeResources[gw_id]['y']
    #             deadline = appsDeadlines[app_num]
    ...
    index = 0
    for k, v in fogDevicesInfo:
        if k not in gatewaysDevices and k is not cloudId:
    ...
    # allocationFile = open(folder_results + "/allocDefinitionMemeticExperimental8.json", "w")
...
def run_simulation():
    ...
    with Pool(processes=1) as pool:  # for AWS  # TODO: change to 128 for AWS
